...
ALLOW_TEMP_MINIGAMES = simbase.config.GetBool('allow-temp-minigames', True)
...
def createMinigame(air, playerArray, trolleyZone, minigameZone = None, previousGameId = ToontownGlobals.NoPreviousGameId, newbieIds = [], startingVotes = None, metagameRound = -1, desiredNextGame = None):
    ...
    if config.GetBool('forbid-surfers-game', False):
        excludeList.append(ToontownGlobals.SurfersGameId)
...
def removeUnreleasedMinigames(startList, increaseChanceOfNewGames = 0):
    ...
    if config.GetBool('forbid-surfers-game', True): # Prevents crash on trolley tracks
        if ToontownGlobals.SurfersGameId in randomList:
            randomList.remove(ToontownGlobals.SurfersGameId)

    return randomList

@magicWord(category=CATEGORY_CHEAT, types=[str, int, int, int])
def requestmimi(minigameName='remove', minigameKeep=False, minigameDiff=None, minigamePG=None):
