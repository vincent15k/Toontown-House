...
class DistributedFurnitureManagerAI(DistributedObjectAI):
    ...
    def moveWallpaperFromAttic(self, index, room):
        ...
        if index >= len(self.atticWallpaper):
            return ToontownGlobals.FM_InvalidIndex
            
        wallpaper = self.atticWallpaper.pop(index)
        if room < len(self.wallpaper):
            oldWallpaper = self.wallpaper.pop(room)
        
            self.atticWallpaper.append(oldWallpaper)
            self.b_setAtticWallpaper(self.atticWallpaper.getBlob())
        
        self.wallpaper.insert(room, wallpaper)
        self.applyWallpaper()
