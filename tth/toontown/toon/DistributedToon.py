...
class DistributedToon(DistributedPlayer.DistributedPlayer, Toon.Toon, DistributedSmoothNode.DistributedSmoothNode, DelayDeletable):
    ...
    def setCatalog(self, monthlyCatalog, weeklyCatalog, backCatalog):
        ...
        if config.GetBool('check-monthly-catalog', True):
            newCatalog = CatalogItemList.CatalogItemList()
            for item in CatalogItemList.CatalogItemList(self.monthlyCatalog):
                if item.loyaltyRequirement():
                    continue
                    
                if 'Garden' in repr(item):
                    continue
                    
                if 'Rental' in repr(item) and not config.GetBool('want-cannon-rental', True):
                    continue
                    
                newCatalog.append(item)
                
            self.monthlyCatalog = newCatalog
